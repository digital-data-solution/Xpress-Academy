"""Thin wrapper around load_verified_questions for PSR Chapter 11
(Petitions and Appeals) — a follow-up to load_civil_service_psr.py
(Chapters 1-2) and the Chapter 3-10 follow-ups, extending the same
bank. See load_civil_service_psr for the full sourcing/verification
writeup; same discipline: 18 questions transcribed from the real
...
this only ever creates or updates these 18 questions."""
...
class Command(BaseCommand):
    help = "Loads civil_service_psr_ch11_18_verified.json into real Question/Choice rows. (Wrapper around load_verified_questions.)"

    def add_arguments(self, parser):
        parser.add_argument(
            "--file",
            default=str(Path(settings.BASE_DIR) / "apps" / "assessment" / "data" / "civil_service_psr_ch11_18_verified.json"),
    ...
    def handle(self, *args, **options):
        call_command(
            ...
            bank_description=(
                ...
                "Currently covers Chapters 1-11; more chapters added as they're verified."
            ),
        )
