"""Thin wrapper around load_verified_questions for PSR Chapter 7
(Training and Staff Development Within and Outside Nigeria) — a
follow-up to load_civil_service_psr.py (Chapters 1-2) and the Chapter
3-6 follow-ups, extending the same bank. See load_civil_service_psr
for the full sourcing/verification writeup; same discipline: 22
...
this only ever creates or updates these 22 questions."""
...
class Command(BaseCommand):
    help = "Loads civil_service_psr_ch7_22_verified.json into real Question/Choice rows. (Wrapper around load_verified_questions.)"

    def add_arguments(self, parser):
        parser.add_argument(
            "--file",
            default=str(Path(settings.BASE_DIR) / "apps" / "assessment" / "data" / "civil_service_psr_ch7_22_verified.json"),
    ...
    def handle(self, *args, **options):
        call_command(
            ...
            bank_description=(
                ...
                "Currently covers Chapters 1-7; more chapters added as they're verified."
            ),
        )
