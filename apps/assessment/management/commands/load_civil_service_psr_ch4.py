"""Thin wrapper around load_verified_questions for PSR Chapter 4
(Emoluments and Increments) — a follow-up to load_civil_service_psr.py
(Chapters 1-2) and load_civil_service_psr_ch3.py (Chapter 3), extending
the same bank rather than forking a new one. See load_civil_service_psr
for the full sourcing/verification writeup; same discipline: 19
questions transcribed from the real scanned document, each
...
class Command(BaseCommand):
    help = "Loads civil_service_psr_ch4_19_verified.json into real Question/Choice rows. (Wrapper around load_verified_questions.)"

    def add_arguments(self, parser):
        parser.add_argument(
            "--file",
            default=str(Path(settings.BASE_DIR) / "apps" / "assessment" / "data" / "civil_service_psr_ch4_19_verified.json"),
    ...
    def handle(self, *args, **options):
        call_command(
            ...
            bank_description=(
                ...
                "Currently covers Chapters 1-4; more chapters added as they're verified."
            ),
        )
